...
class Blockchain:
    ...
    def add_block(self, validator):
        ...
        print(f"✅ Block {new_block.index} added. Previous hash: {new_block.previous_hash}")
        self.pending_transactions = []
    ...
    def validate_and_add_block(self):
        ...
        if validator:
            print(f"✅ Validator {validator} selected for block {len(self.chain)}")
            self.add_block(validator)
            print(f"🛠️ Block added with transactions: {self.pending_transactions}")
        else:
